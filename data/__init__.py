...
from data.cuhk_dataset import cuhk_pede_train, cuhk_pede_caption_eval, cuhk_pede_retrieval_eval, \
    cuhk_pede_trainset_eval, mix_pede_train
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset == 'caption_cuhk':
        train_dataset = cuhk_pede_train(transform_train, config['image_root'],
                                        return_masks=config.get('return_masks', False),
                                        return_gt=config.get('return_gt', False),
                                        missing_mode=config.get('missing_mode', 'easy'))
