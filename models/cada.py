...
from models.MFC import FeatureGenerator
...
def detect_missing_modalities_input_only(image_tensor: torch.Tensor, text_input: dict, tokenizer):
    ...
    def denormalize_image_tensor(image_tensor, mean, std):
        mean = torch.tensor(mean).to(image_tensor.device).view(1, -1, 1, 1)
        std = torch.tensor(std).to(image_tensor.device).view(1, -1, 1, 1)
    ...
    image_mean = image_tensor.view(B, -1).mean(dim=1)
    is_image_missing = image_mean < 0.01
    ...
    is_text_missing = special_mask.all(dim=1)
...
class MGCN(nn.Module):
    ...
    def forward(self, image, caption, idx):
        ...
        loss_mfc = torch.tensor(0.0, device=image.device)
        is_image_missing, is_text_missing = detect_missing_modalities_input_only(image, text, self.tokenizer)
        image_cls_feat = image_embeds[:, 0, :]
        text_cls_feat = text_embeds[:, 0, :]
        if is_image_missing.any() or is_text_missing.any():
            completed_image_feat, completed_text_feat, loss_mfc = self.feature_generator(image_cls_feat, text_cls_feat, is_image_missing, is_text_missing)

            image_cls_feat = completed_image_feat
            text_cls_feat = completed_text_feat

            if is_image_missing.any():
                image_missing_indices = is_image_missing.nonzero(as_tuple=True)[0]
                image_embeds[image_missing_indices, 0, :] = completed_image_feat[image_missing_indices]
            if is_text_missing.any():
                text_missing_indices = is_text_missing.nonzero(as_tuple=True)[0]
                text_embeds[text_missing_indices, 0, :] = completed_text_feat[text_missing_indices]
            text_feats = self.text_proj(text_cls_feat)
            image_feats = self.vision_proj(image_cls_feat)
        ...
        return global_loss, local_loss, ara_loss, loss_mfc
